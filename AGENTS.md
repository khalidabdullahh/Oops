...
*   **Visual Themes**: `DESERT` (Levels 1-3), `SHADOW` (Levels 4-6), and `VOID` (Levels 7-10) with custom background layers and matching HUD styling.
...
*   **Opening Cinematic/Intro Animation**: The `runOpeningAnimation()` helper is currently declared conceptually but not implemented. The game starts immediately.
*   **Wiring Platform Themes**: Platform classes (`Platform`, `Spike`, `Saw`, `Exit`) still draw using static colors from the legacy `PALETTE` constant instead of reading from `activeTheme`.
