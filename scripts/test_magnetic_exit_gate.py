...
tmp_dir = tempfile.mkdtemp(prefix='chrome_mag_exit_')
...
# 1. Start GameScene (World 1 Level 2 - where gate is at x=900, y=437)
ws_send(s, {'id': 10, 'method': 'Runtime.evaluate', 'params': {'contextId': ctx_id, 'expression': '''
(() => {
  window.game.scene.start("GameScene", { world: 0, level: 1, deaths: 0 });
})()
'''}})
time.sleep(0.5)

# 2. Teleport hero to near the gate (x=840, y=410, moving towards and jumping over gate)
ws_send(s, {'id': 20, 'method': 'Runtime.evaluate', 'params': {'contextId': ctx_id, 'expression': '''
(() => {
  const g = window.game.scene.getScene("GameScene");
  g.player.x = 845;
  g.player.y = 400;
  g.player.setVelocityX(250);
  g.player.setVelocityY(-300); // jumping towards/over gate
})()
'''}})
time.sleep(0.6)

# 3. Check level completion / magnetic suction status
...
(() => {
  ...
  return JSON.stringify({
    ...
    gateX: Math.round(g.exitGate.x),
    gateY: Math.round(g.exitGate.y)
  });
})()
''', 'returnByValue': True}})
...
while time.time() - st < 3.0:
    try:
        ...
        if mid == 30:
            ...
            print(f'MAGNETIC GATE TEST RESULT:', val)
    except socket.timeout:
        break
...
print('MAGNETIC EXIT GATE TEST FINISHED!')
