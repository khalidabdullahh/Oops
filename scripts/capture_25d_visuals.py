...
tmp_dir = tempfile.mkdtemp(prefix='chrome_25d_test2_')
...
# Ensure SaveManager marks intro seen and jump to GameScene Level 1
eval_js('''(() => {
    localStorage.setItem("oops_intro_seen", "true");
    window.game.scene.stop("IntroScene");
    window.game.scene.stop("WorldSelectScene");
    window.game.scene.start("GameScene", { world: 0, level: 0, deaths: 0, levelDeaths: 0 });
})()''')

time.sleep(1.0)

# Verify GameScene active
g_active = eval_js('window.game.scene.isActive("GameScene")')
print("GameScene isActive:", g_active)

capture_screenshot('visual_25d_landscape.png', 844, 390, True)
capture_screenshot('visual_25d_portrait.png', 390, 844, True)
