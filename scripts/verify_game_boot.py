...
tmp_dir = tempfile.mkdtemp(prefix='chrome_boot_')
...
# Check DOM & Game status
ws_send(s, {'id': 10, 'method': 'Runtime.evaluate', 'params': {
  'expression': '''(() => {
    const modal = document.getElementById("feedback-modal");
    const canvas = document.querySelector("canvas");
    const activeScenes = window.game ? window.game.scene.getScenes(true).map(s => s.scene.key) : [];
    return JSON.stringify({
      modalStyleDisplay: modal ? modal.style.display : 'no modal',
...
while time.time() - st < 3.0:
    try:
        ...
        if mid == 10:
            ...
            print('BOOT VERIFICATION RESULT:', val)
