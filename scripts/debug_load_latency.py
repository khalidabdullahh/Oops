...
tmp_dir = tempfile.mkdtemp(prefix='chrome_latency_')
...
# Poll until game is booted
game_booted = False
check_count = 0
while time.time() - t0 < 10.0 and not game_booted:
    check_count += 1
    ws_send(s, {'id': 100 + check_count, 'method': 'Runtime.evaluate', 'params': {
        'expression': '''(() => {
            return {
                phaserLoaded: typeof Phaser !== 'undefined',
                gameRunning: !!window.game && window.game.isBooted,
    ...
    while time.time() - st < 0.5:
        try:
            ...
            if not msg:
                break
            if msg.get('id') == 100 + check_count:
                res = msg.get('result', {}).get('result', {}).get('value', {})
                if res.get('gameRunning') and len(res.get('activeScenes', [])) > 0:
                    print(f"GAME BOOTED IN {time.time() - t0:.2f}s! Active scenes:", res.get('activeScenes'))
                    game_booted = True
                    break
                else:
                    print(f"Tick {check_count} ({time.time() - t0:.2f}s):", res)
