...
tmp_dir = tempfile.mkdtemp(prefix='chrome_fb_vis_')
...
time.sleep(1.0)
# Capture World Map screenshot with feedback button
ws_send(s, {'id': 10, 'method': 'Page.captureScreenshot', 'params': {'format': 'png'}})

# Start GameScene and capture gameplay screenshot with feedback button
ws_send(s, {'id': 20, 'method': 'Runtime.evaluate', 'params': {'contextId': ctx_id, 'expression': 'window.game.scene.start("GameScene", { world: 0, level: 0, deaths: 0 })'}})
time.sleep(1.0)
...
while time.time() - st < 3.0:
    try:
        ...
        if mid == 10:
            ...
            if data:
                with open('/Users/khalidabdullah/AntiGravity/Oops!/docs/feedback_world_map.png', 'wb') as f:
                    f.write(base64.b64decode(data))
                print('Saved docs/feedback_world_map.png')
        elif mid == 30:
            data = msg.get('result', {}).get('data')
            if data:
                with open('/Users/khalidabdullah/AntiGravity/Oops!/docs/feedback_gameplay.png', 'wb') as f:
                    f.write(base64.b64decode(data))
                print('Saved docs/feedback_gameplay.png')
    except socket.timeout:
        break
