...
tmp_dir = tempfile.mkdtemp(prefix='chrome_img_fb_')
...
# 1. Start GameScene (World 1 Level 4)
ws_send(s, {'id': 10, 'method': 'Runtime.evaluate', 'params': {'contextId': ctx_id, 'expression': 'window.game.scene.start("GameScene", { world: 0, level: 3, deaths: 7 })'}})
time.sleep(0.8)
...
(() => {
  ...
  return JSON.stringify({ btnWidth: Math.round(rect.width), btnHeight: Math.round(rect.height) });
})()
''', 'returnByValue': True}})

# 3. Test Auto-Snap Game Screen
...
(() => {
  ...
  const prevBox = document.getElementById("fb-preview-container");
  const isVisible = prevBox && !prevBox.classList.contains("hidden");
  const textName = document.getElementById("fb-preview-name")?.textContent;
  const imgSrc = document.getElementById("fb-preview-img")?.src;
  
  return JSON.stringify({ isPreviewVisible: isVisible, textName, hasValidSrc: imgSrc && imgSrc.startsWith("data:image/") });
})()
''', 'returnByValue': True}})

# 4. Fill message and submit to test attached image in GitHub issue URL
ws_send(s, {'id': 40, 'method': 'Runtime.evaluate', 'params': {'contextId': ctx_id, 'expression': '''
(() => {
  document.getElementById("fb-category").value = "🐛 Bug Report";
  document.getElementById("fb-name").value = "VisualTester";
  document.getElementById("fb-message").value = "The spikes popped up suddenly! Check the attached screenshot.";
  ...
  return JSON.stringify({
    capturedUrl: decodeURIComponent(capturedUrl),
...
print('IMAGE ATTACHMENT FEEDBACK TEST COMPLETED SUCCESSFULLY!')
