...
tmp_dir = tempfile.mkdtemp(prefix='chrome_ad_modal_')
...
# Start GameScene
eval_js('window.game.scene.start("GameScene", { world: 0, level: 0, deaths: 6, levelDeaths: 6, skipOfferUnlocked: false });')
time.sleep(0.5)

# Trigger 7th death to show popup
eval_js('window.game.scene.getScene("GameScene").onPlayerDie();')
time.sleep(0.6)

# Capture Portrait Popup
capture_screenshot('mobile_portrait_ad_offer.png', 390, 844, True)

# Capture Landscape Popup
capture_screenshot('mobile_landscape_ad_offer.png', 844, 390, True)

# Decline popup and capture persistent skip button in Portrait deck
eval_js('document.getElementById("btn-ad-decline").click();')
time.sleep(0.5)
capture_screenshot('mobile_portrait_skip_unlocked.png', 390, 844, True)
capture_screenshot('mobile_landscape_skip_unlocked.png', 844, 390, True)
