...
tmp_dir = tempfile.mkdtemp(prefix='chrome_err_')
...
time.sleep(1.0)
ws_send(s, {'id': 10, 'method': 'Runtime.evaluate', 'params': {
  'expression': '''(() => {
    const modal = document.getElementById("feedback-modal");
    const container = document.getElementById("game-container");
    const canvas = document.querySelector("canvas");
    return JSON.stringify({
      modalComputedDisplay: modal ? window.getComputedStyle(modal).display : 'no modal',
      modalClassList: modal ? Array.from(modal.classList) : [],
      hasCanvas: !!canvas,
...
while time.time() - st < 3.0:
    try:
        ...
        if not msg:
            break
        if msg.get('method') == 'Runtime.consoleAPICalled' or msg.get('method') == 'Runtime.exceptionThrown':
            logs.append(msg)
        if msg.get('id') == 10:
            val = msg.get('result', {}).get('result', {}).get('value')
            print('DOM STATUS:', val)
    except socket.timeout:
        break

print('CONSOLE/EXCEPTION LOGS:', json.dumps(logs, indent=2))
